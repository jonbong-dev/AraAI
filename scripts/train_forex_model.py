...
def log_dataset_to_comet(experiment, df, asset_type):
    """Log per-symbol dataset stats and aggregate metadata."""
...
def train_forex_model(
    ...
    experiment = init_comet(
        project_name="meridian-ai-forex-v5",
    ...
    log_dataset_to_comet(experiment, data, "forex")
    ...
    print(f"\nTraining unified forex model (up to {max_time_minutes}min / {epochs} epochs)...")
